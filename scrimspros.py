...
# 1) Leer timestamps ya en el Excel
...
# 2) Detectar scrims NUEVAS, filtrar solo por timestamp
def detect_scrims_by_timestamp(existing_ts):
    ...
    for tag in PRO_TAGS:
        for b in get_battlelog(tag):
            ...
            ts = b.get("battleTime")
            if not ts or ts in existing_ts or ts in seen_new_ts:
                continue
            seen_new_ts.add(ts)
            ...
            res = info.get("result","")
            if   res == "victory": winner = "Equipo 1"
            elif res == "defeat":  winner = "Equipo 2"
            elif res == "draw":    winner = "Empate"
            else:                  continue

            scrims_by_map[map_name].append({
                "battle_time":  ts,
                "brawlers_e1":  b_e1,
                "brawlers_e2":  b_e2,
                "names_e1":     n_e1,
...
# 3) Guardar en Excel (incluye Timestamp como última columna)
def save_scrims(scrims_by_map, filename="scrims_actualizado.xlsx"):
    ...
    for map_name, entries in scrims_by_map.items():
        ...
        if ws.max_row < 3:
            ...
            for col, h in enumerate(headers, start=1):
                ...
                c.border = thin
                if col <= 3:      c.fill = blue
                elif 4 <= col <=6:c.fill = red

        ts_col     = ws.max_column
        start_row  = ws.max_row + 1 if ws.max_row >= 4 else 4

        for i, e in enumerate(entries, start=start_row):
            # brawlers, ganador, nombres...
            for j,b in enumerate(e["brawlers_e1"], start=1):
                c = ws.cell(row=i, column=j, value=b); c.fill, c.border = blue, thin
            for j,b in enumerate(e["brawlers_e2"], start=4):
                c = ws.cell(row=i, column=j, value=b); c.fill, c.border = red, thin
            wc = ws.cell(row=i, column=7, value=e["winner"])
            wc.fill = {"Equipo 1":blue, "Equipo 2":red}.get(e["winner"], gray)
            wc.font, wc.border = bold, thin
    ...
    wb.save(filename)
    print(f"✅ Excel actualizado (filtro solo por timestamp): {filename}")

# ────── BLOQUE PRINCIPAL ──────
if __name__ == "__main__":
    archivo     = "scrims_actualizado.xlsx"
    existing_ts = load_existing_timestamps(archivo)
    nuevas      = detect_scrims_by_timestamp(existing_ts)

    if nuevas:
        save_scrims(nuevas, archivo)
    else:
        print("⚠️ No hay scrims nuevas (solo timestamp).")
