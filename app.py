...
def score_adjusted(w, n, m=10, z=1.96):
    """
    Wilson score penalizado según número de partidas:
    m = partidas "suficientes"
    """
    raw    = score_wilson(w, n, z)
    factor = n / (n + m)
    return raw * factor
...
@app.callback(
    Output("main-dropdown","options"), Output("main-dropdown","value"),
...
# ... callbacks comp1, comp2, r1, r2, r3 idénticos a tu código previo ...
...
def update_tables(m, main, c1, c2, r1, r2, r3):
    ...
    if main:
        ...
        for b in comps:
            ...
            comp_data.append({"brawler":b,"games":int(g),"wins":int(v),
                              "wr":round(0 if g==0 else v/g*100,1)})
        ...
        for b in rivs:
            ...
            riv_data.append({"brawler":b,"games":int(g),"wins_vs":int(v),
                             "wr_vs":round(0 if g==0 else v/g*100,1)})
        riv_data.sort(key=lambda x:(-x["games"],-x["wr_vs"]))

    return mw, comp_data, riv_data

# ————————————— Draft Assistant Callbacks con score_adjusted —————————————
...
def suggest_first(mapa, ba, bb):
    ...
    df = df[~df.apply(lambda r: any(b in banned for b in r["team1"]+r["team2"]), axis=1)]
    ...
    scored = [
        (b,
         score_adjusted(v["v"], v["g"], m=10),
         v["g"],
         score_wilson(v["v"], v["g"])
        )
    ...
    suger = html.Ul([
        html.Li(f"{i+1}. {b} → ajustado {adj*100:.1f}% "
                f"({g} partidas; raw {raw*100:.1f}%)")
        for i,(b,adj,g,raw) in enumerate(top5)
    ])
    opts = [{"label":b,"value":b} for b,_,_,_ in scored]
    return suger, opts
...
def suggest_23(mapa, ba, bb, p1):
    ...
    banned = set(ba or []) | set(bb or [])
    df = df[~df.apply(lambda r: any(b in banned for b in r["team1"]+r["team2"]), axis=1)]
    df = df[df.apply(lambda r: p1 in (r["team1"]+r["team2"]), axis=1)]
...
def suggest_45(mapa, ba, bb, p1, p23):
    ...
    banned = set(ba or []) | set(bb or [])
    df = df[~df.apply(lambda r: any(b in banned for b in r["team1"]+r["team2"]), axis=1)]
    df = df[df.apply(lambda r: last in (r["team1"]+r["team2"]), axis=1)]
...
def suggest_last(mapa, ba, bb, p1, p23, p45):
    ...
    banned = set(ba or []) | set(bb or [])
    df = df[~df.apply(lambda r: any(b in banned for b in r["team1"]+r["team2"]), axis=1)]
    df = df[~df.apply(lambda r: any(p in (r["team1"]+r["team2"]) for p in picks), axis=1)]
